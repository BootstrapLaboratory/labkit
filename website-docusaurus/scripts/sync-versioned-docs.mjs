...
async function writeVersionDocs(version) {
  ...
  await writeFile(
    ...
    [
      ...
      'title: "Docs"',
      'sidebar_label: "Docs"',
      `description: ${JSON.stringify(`Documentation for Labkit ${version}.`)}`,
      "---",
      "",
      `You are viewing archived documentation for Labkit ${version}.`,
      "",
  ...
  if (tree.tutorialItems === undefined || tree.tutorialItems.length === 0) {
    await writeFile(
      ...
      [
        ...
        `description: ${JSON.stringify(`Tutorial availability for Labkit ${version}.`)}`,
        "---",
        "",
        `The tutorial was not published for Labkit ${version}.`,
        "",
